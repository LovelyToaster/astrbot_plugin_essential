...
class Main(Star):
    ...
    async def epic_free_game(self, message: AstrMessageEvent):
        """EPIC 喜加一"""
        url = "https://api.nxvav.cn/api/epic/"

        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status != 200:
                    return CommandResult().message("获取 Epic 喜加一失败喵~").use_t2i(False)

                data = await resp.json()

        games = data.get("data", [])
        if not games:
            return CommandResult().message("目前没有获取到 Epic 喜加一信息喵~").use_t2i(False)
        ...
        for game in games:
            ...
            link = game["link"]
            start = game["free_start"]
            end = game["free_end"]

            if game.get("is_free_now"):
                now_free.append(
                    f"🎮 **{title}**\n"
                    f"🕒 免费时间：{start} ~ {end}\n"
                    f"🔗 {link}\n"
        ...
        msg_parts = ["🎁 EPIC 喜加一情报\n"]

        if now_free:
            msg_parts.append("🔥 **正在免费：**\n" + "\n".join(now_free))

        if upcoming:
            msg_parts.append("📅 **即将免费：**\n" + "\n".join(upcoming))
